...
export default class NasiLapolaScene extends Phaser.Scene {
  ...
  // UI Components - akan diatur oleh CSS
  ...
  private layoutConfig = {
    ...
    // Ingredients panel
    ingredientsPanelWidth: 260,
    ingredientsPanelX: 0, // Will be calculated
    ingredientsPanelY: 70,
    ingredientsPanelHeight: 0, // Will be calculated
    ...
    stagingAreaX: 50,
    stagingAreaY: 100,
    stagingAreaWidth: 150,
    stagingAreaHeight: 120
  };
  ...
  private calculateLayout() {
    ...
    this.layoutConfig.ingredientsPanelHeight = gameHeight - this.layoutConfig.headerHeight - this.layoutConfig.dialogPanelHeight - 80;
  ...
  private createCookingArea() {
    ...
    stagingGraphics.lineStyle(2, 0xFFD700, 0.6);
  ...
  private createIngredientsPanel() {
    ...
    panelBg.fillRoundedRect(0, 0, this.layoutConfig.ingredientsPanelWidth, this.layoutConfig.ingredientsPanelHeight, 20);
    panelBg.lineStyle(2, 0x8B4513, 0.8);
    panelBg.strokeRoundedRect(0, 0, this.layoutConfig.ingredientsPanelWidth, this.layoutConfig.ingredientsPanelHeight, 20);
    this.ingredientsPanel.add(panelBg);

    // Panel header
    const headerBg = this.add.graphics();
    headerBg.fillStyle(0x4A3428, 0.9);
    headerBg.fillRoundedRect(10, 10, this.layoutConfig.ingredientsPanelWidth - 20, 40, 8);
    this.ingredientsPanel.add(headerBg);

    // Panel title
    const panelTitle = this.add.text(this.layoutConfig.ingredientsPanelWidth/2, 30, "BAHAN & ALAT", {
      fontSize: '14px',
      fontFamily: 'Chewy, cursive',
      color: '#FFE4B5',
      align: 'center',
    ...
    this.ingredientsPanel.add(panelTitle);

    // Menu toggle button (reduced size)
    this.menuToggleButton = this.add.image(this.layoutConfig.ingredientsPanelWidth - 30, 30, "menu_normal")
      .setScale(0.4) // Further reduced from 0.5
      .setInteractive()
      .on('pointerover', () => {
    ...
    // Create ingredients with manual positioning
    this.createIngredients();
  }

  private createIngredients() {
    const ingredients = [
      { key: "Kacang", name: "Kacang", scale: 0.18 },
      { key: "Kelapa", name: "Kelapa", scale: 0.16 },
      { key: "Beras", name: "Beras", scale: 0.18 },
      { key: "Garam", name: "Garam", scale: 0.14 },
      { key: "Sepatula", name: "Spatula", scale: 0.16 },
      { key: "PanciAir2", name: "Panci", scale: 0.14 },
    ...
    ingredients.forEach((ingredient, i) => {
      ...
      itemBg.fillRoundedRect(x - 35, y - 25, 70, 50, 8);
      itemBg.lineStyle(1, 0x8B4513, 0.4);
      itemBg.strokeRoundedRect(x - 35, y - 25, 70, 50, 8);
      this.ingredientsPanel.add(itemBg);

      // Item image
      const item = this.add.image(x, y - 5, ingredient.key)
        .setInteractive()
      ...
      // Item label
      const label = this.add.text(x, y + 20, ingredient.name, {
        fontSize: '9px',
        fontFamily: 'Chewy, cursive',
        color: '#FFE4B5',
        align: 'center',
  ...
  private initDragAndDrop() {
    this.input.on("drag", (pointer, gameObject, dragX, dragY) => {
      ...
      gameObject.setScale(gameObject.scale * 1.15);
    ...
    this.input.on("dragend", (pointer, gameObject, dropped) => {
      ...
      const originalScale = gameObject.getData('originalScale') || 
        (gameObject.name === 'Kacang' ? 0.18 : 
         gameObject.name === 'Kelapa' ? 0.16 : 
         gameObject.name === 'Beras' ? 0.18 : 
         gameObject.name === 'Garam' ? 0.14 : 
         gameObject.name === 'Sepatula' ? 0.16 : 0.15);
